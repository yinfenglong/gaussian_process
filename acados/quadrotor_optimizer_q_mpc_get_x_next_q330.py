    ...
    def trajectory_command_callback(self, data):
        ...
        if data.size != len(temp_traj):
            ...
            for i in range(data.size):
                ...
                # print("trajectory_path", temp_traj[i].x, temp_traj[i].y, temp_traj[i].z,\
                #     temp_traj[i].roll, temp_traj[i].pitch, temp_traj[i].yaw,\
                #         temp_traj[i].vx, temp_traj[i].vy)
                # print("vz", temp_traj[i].vz)
    ...
    def mpc_estimation_loop(self,):
        ...
        if self.trajectory_path is not None and self.current_state is not None:
            ...
            if status != 0:
                ...
                # self.att_command.thrust = mpc_u_[3] / self.g_ * 0.59
                # q330, circle, control_offset=0.46
                self.att_command.thrust = mpc_u_[3] / self.g_ * 0.46
