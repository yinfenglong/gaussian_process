...
class UAVSubNpy(object):
    ...
    def robot_odom_callback(self, msg):
        ...
        if vel is not None:
            self.uav_pose = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z,
                    ...
                    msg.pose.pose.orientation.z, vel[0], vel[1], vel[2] ])
        else:
            pass
        ...
        if vel is not None:
            if msg.pose.orientation.w > 0:
                self.uav_pose = np.array([msg.pose.position.x, msg.pose.position.y, msg.pose.position.z,
                        ...
                        msg.pose.orientation.z, vel[0], vel[1], vel[2] ])
    ...
    def velocity_estimation(self, ):
        if not self.is_velocity_init:
            ...
            if dt>=0.01:
                vel = (self.current_position - self.last_position)/(1e-5 + dt)
                vel = 0.8 * vel + 0.2 * self.last_velocity

                self.last_velocity = vel
                self.previous_time = self.current_time
                self.last_position = self.current_position
                print("right dt is {}".format(dt))

                return vel
            else:
                print("error dt is {}".format(dt))
                return None
    ...
    if is_rate_cmd:
        while not rospy.is_shutdown():
            ...
            if current_time_ - sub_obj.cmd_timer > 2. or current_time_ - sub_obj.pose_timer > 2.:
                # safe the data
                np.save('./q330/exp_data_20210913_1_hover_local_qw.npy', data_list)
                # np.save('./gazebo/exp_data_20210914_gazebo_vel_est_local.npy', data_list)
                break
