...
class GpPredict(object):
    ...
    def gp_mean_publish_process(self,):
        ...
        if self.uav_pose is not None:
            ...
            # gp predict
            gp_vx_b = self.gpMPCVx.predict_mean(np.array([v_b[0]]), \
                self.gpMPCVx.likelihood_pred, self.gpMPCVx.model_to_predict)[0]
            gp_vy_b = self.gpMPCVy.predict_mean(np.array([v_b[1]]), \
                self.gpMPCVy.likelihood_pred, self.gpMPCVy.model_to_predict)[0]
            gp_vz_b = self.gpMPCVz.predict_mean(np.array([v_b[2]]), \
                self.gpMPCVz.likelihood_pred, self.gpMPCVz.model_to_predict)[0]
